# secret-breach-response

**Adapter surface**: `cursor`
**Authority**: `AGENTS.md#Agent Behavior Protocol`
**Mode**: `STOP`
**Canonical source**: `.windsurf/skills/secret-breach-response/SKILL.md`

Read `.windsurf/skills/secret-breach-response/SKILL.md` in full before invoking this skill. The canonical
skill body, trigger conditions, escalation rules, and success criteria
live there.

This file exists only so `cursor` can discover the skill without
forking `.windsurf/skills/`.
