...
| **A3** | Cluster defaults: private endpoint configurable and secure by default, system/workload node pools, NetworkPolicy deny-default | NO — depends on A2 |
...
| A2 | ✅ **shipped** — Bootstrap/live Terraform split | `templates/infra/terraform/{gcp,aws}/bootstrap/` directories provision the foundation that the live layer assumes exists: state bucket (versioned, KMS-encrypted, prevent_destroy, soft-delete window), KMS keyring + key, container registry (Artifact Registry on GCP / ECR on AWS — immutable tags, lifecycle policies for untagged + tagged retention), and AWS DynamoDB lock table (PAY_PER_REQUEST + PITR). Both bootstraps use LOCAL state by design (chicken-and-egg with the very bucket they create — `main.tf` deliberately omits the `backend` block). `.gitignore` per bootstrap dir prevents committing local state files. `templates/infra/terraform/README.md` documents the bootstrap-then-live workflow including output capture into `backend-configs/<env>.hcl` and a migration path for adopters who CLI-provisioned state buckets via the legacy runbook. `docs/runbooks/terraform-state-bootstrap.md` updated with the two-bootstrap-paths table at the top. Contract test `templates/service/tests/test_terraform_bootstrap_contract.py` (15 cases) enforces: bootstrap directory exists with main.tf/variables.tf/state.tf, NO `backend` block in bootstrap (LOCAL state), state-bucket resource exists with `prevent_destroy = true`, `output.tfstate_bucket` exists for both clouds, `.gitignore` covers `terraform.tfstate` + `.terraform/`, README documents bootstrap/live/chicken-and-egg/backend-configs vocabulary, AND **no resource collision** between bootstrap and live (state bucket / KMS keyring / DDB lock table cannot be redeclared in live without lifecycle conflict). `validate-templates.yml::terraform-validate` job extended to validate both layers per cloud (4 validates total). `terraform validate` passes on all 4. |
| A3 | ✅ **shipped** — Cluster defaults: private endpoint configurable and secure by default, system/workload split, deny-default NetworkPolicy | GCP (`templates/infra/terraform/gcp/`): `enable_private_endpoint` is a variable (default true after enterprise hardening) wired into `private_cluster_config`; `master_authorized_networks` accepts a list of CIDR+display_name objects rendered via `dynamic` block for explicit dev relaxation. `compute.tf` split into `system` pool (small, no taint, runs kube-system + monitoring) and `workload` pool (taint `workload-type=ml-services:NoSchedule`); workload pods MUST tolerate the taint or stay unschedulable on workload nodes. AWS (`templates/infra/terraform/aws/`): `aws_eks_node_group.system` + `aws_eks_node_group.workload` mirror GCP with the same taint contract; reaffirms PR-R2-6's `allow_public_endpoint=false` default. K8s base: NEW `networkpolicy-deny-default.yaml` selects ALL pods in the namespace with empty `podSelector: {}` and zero ingress/egress rules — combined with the per-service allow rules in `networkpolicy.yaml`, the composition is deny-default + allow-explicit; baseline is wired into `kustomization.yaml`. `templates/k8s/base/deployment.yaml` ships the matching `tolerations` block (`key: workload-type, value: ml-services, effect: NoSchedule`) so ML pods can be scheduled on the tainted workload pool. Contract test `templates/service/tests/test_cluster_defaults_contract.py` enforces: `enable_private_endpoint` is a variable + cluster references `var.*` (not literal); AWS `allow_public_endpoint` default remains false; both clouds have system + workload pools; workload pool has `NO_SCHEDULE` taint; system pool has NO taint; deny-default NetworkPolicy exists with empty selector + both policyTypes + zero rules; deny-default in kustomization; base Deployment has matching toleration. **Phase A is now ✅ COMPLETE** (A1-A5 + A5b all shipped). |
| A-parity | ✅ **shipped** — AWS/GCP live-layer parity gap closed | User-reported gap: AWS shipped `secrets.tf`, `logging.tf`, `ecr.tf`, `iam-roles-split.tf` while GCP did not have equivalents. Fixed in this commit: NEW `templates/infra/terraform/gcp/secrets.tf` (Secret Manager, one entry per service×secret_name Cartesian product, CMEK via dedicated keyring, per-secret IAM binding for the runtime SA), NEW `templates/infra/terraform/gcp/logging.tf` (Cloud Logging buckets per service with `var.log_retention_days` overriding GCP's fixed 30d default, CMEK via logs key, `google_billing_budget` gated behind `var.billing_account` with 80%/100% thresholds matching AWS Budgets), NEW `templates/infra/terraform/gcp/kms.tf` (live-layer keyring `${project}-workload-${env}` with two crypto keys: `secrets` + `logs`, both with 90d rotation + 30d destroy window + IAM bindings to the Secret Manager and Cloud Logging service agents). Variables added on GCP: `service_names`, `secret_names`, `log_retention_days`, `billing_account`, `budget_notification_emails`. Defaults match AWS so a single overlay tfvars works for either cloud. Contract test `templates/service/tests/test_terraform_cloud_parity.py` (14 cases) enforces: secret store parity (Cartesian product on both), logging retention parity (no "Never expire" defaults), budget alert parity, CMEK parity (secrets + logs reference customer-managed keys, not provider-managed defaults), shared variable surface across both clouds, AND `secret_names` defaults match exactly so `["api_key","admin_api_key","mlflow_password"]` works in both. Also fixed the CI tfsec failure: replaced TF 1.5+ `check` block in `aws/network.tf` (rejected by tfsec's HCL parser per aquasecurity/tfsec#1994) with a `terraform_data` resource + `lifecycle.precondition` — supported by every parser since TF 1.4 with identical plan-time error semantics. |
...
The sub-table is the canonical Phase A status. The 7-commit
sequence on A5 reflects the run-by-run debugging the workflow
itself produced — each fix narrowed the failure surface by one
step further into the chain until the whole chain went green.
This is the intended operating shape of an E2E bring-up: each
green step proves a contract; each red step exposes one new
contract that needs fixing. The artifact is not just the green
workflow but the audit trail of contracts proven along the way.
