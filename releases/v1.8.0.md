...
- Pod Security Standards `restricted` baseline on namespaces (closed in v1.8.1).
- SBOM attestation via Cosign + Syft in deploy workflows (closed in v1.8.1).
- Explicit ThreadPool sizing guidance (closed in v1.8.1).
- Intersectional fairness via `run_fairness_audit(intersectional=True)` (closed in v1.8.1).
- Batch scoring CronJob + GitOps ADR + DORA metrics (closed in v1.9.0).
