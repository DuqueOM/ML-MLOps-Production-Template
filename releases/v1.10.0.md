...
- Alert firing via Prometheus + Pushgateway (Stage 2 of PR-R2-9) — closed in v1.11.0.
- Closed-loop heartbeat monitoring convergence with audit log — closed in v1.11.0.
- ADR-018 memory plane + ADR-019 CI self-healing Phase 1 runtime — closed in v1.12.0.
- Full `git log --all -p | gitleaks detect --pipe` history sweep (R4 finding H6) — closed in the v1.12 audit-r4 sprint.
