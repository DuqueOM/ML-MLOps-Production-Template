...
template's stated guarantees (D-01..D-35 invariants, agentic governance,
...
| Phase 2 — Template correctness | **Done** | `05c7d19` → `a07e527` (+ ci-examples.yml in this commit) | All 6 sub-commits effective: §2.1 ServiceA→{@ service_name @} parametrization; §2.2 test_api.py real suite (8 classes / 26 tests, conftest fixtures); §2.3 test_training.py stubs→pytest.skip + new determinism gate; §2.4 test_schema_evolution.py + CI pair-change guard; §2.5 scaffold-e2e smoke chain (install + snapshot + pytest); §2.6 coverage badge wired to common_utils + service/app + templates/tests/unit/. |
